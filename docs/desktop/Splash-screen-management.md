...
Since the splash screen content is managed using the `UI Composition` engine in order to add a content to the splash screen is enough to define a [partial view](UI-Composition#automatic-aka-partial-regions) named `SplashScreenContent`:
